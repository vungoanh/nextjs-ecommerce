    ...
    <div className='card text-black cursor-pointer card-compact m-3 w-80 bg-white shadow-xl relative'>
      ...
      <div className='card-body'>
        ...
        <p className='font-semibold'
           onClick={() => router.push(`/product/product-detail/${_id}`)}>{`Rs ${productPrice}`}</p>
        <div className='card-actions justify-end z-20'>
          <button onClick={AddToCart} className='btn  btn-circle btn-ghost '><BsCartPlus
            className='text-2xl text-orange-600 font-semibold' /></button>
          <button onClick={AddToBookmark} className='btn btn-circle btn-ghost absolute top-0 right-0 '><MdFavorite
            className='text-2xl text-orange-600 font-semibold' /></button>
        </div>
      </div>
    </div>
