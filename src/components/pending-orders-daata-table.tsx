...
import { setBookmark } from '@/utils/book-mark'
...
export default function PendingOrdersDaataTable() {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row: Order) => (
        ...
        <button onClick={() => updateOrderStatus(row?._id)}
                className=' w-20 py-2 mx-2 text-xs text-green-600 hover:text-white my-2 hover:bg-green-600 border border-green-600 rounded transition-all duration-700'>Delivered</button>
